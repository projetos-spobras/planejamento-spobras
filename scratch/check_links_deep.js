...
async function test() {
    ...
    const ids = [1625, 1626, 1627, 2022, 2023];
    for (const id of ids) {
        const res = await fetch(`${baseUrl}/api/ContratoEmpreendimentos?idEmpreendimento=${id}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await res.json();
        const items = data.itens || data;
        console.log(`Links for project ${id}:`, items.map(l => l.idContrato));
    }
}
